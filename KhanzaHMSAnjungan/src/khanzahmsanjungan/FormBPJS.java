...
//        int anjunganHeight = (int) (Math.round(ySize * 0.80));
        anjunganHeight = (int) (Math.round(screen.height * 0.90));
                ...
                if(Sequel.cariInteger("select count(no_rkm_medis) from pasien where no_rkm_medis=?",TCari.getText().trim())>0){
                     if(validasiregistrasi.equals("Yes")){
                         if(Sequel.cariInteger("select count(no_rkm_medis) from reg_periksa where no_rkm_medis=? and status_bayar='Belum Bayar' and stts<>'Batal'",TCari.getText())>0){
                             JOptionPane.showMessageDialog(rootPane,"<html><div align='center'><font size='5' face='Tahoma' color='#825082'>Maaf, pasien pada kunjungan sebelumnya memiliki tagihan yang belum di closing.\\nSilahkan konfirmasi dengan pihak kasir.. !!</font></div></html>");
                         }else{
                             try {
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        sikulixScreen form;
        String noKTP = Sequel.cariIsi("select pasien.no_ktp from pasien where pasien.no_rkm_medis=?",TCari.getText());
        String noHP = Sequel.cariIsi("select pasien.no_tlp from pasien where pasien.no_rkm_medis=?",TCari.getText());
        
        // String tanggal lahir
        String tanggal_lahir = Sequel.cariIsi("select pasien.tgl_lahir from pasien where pasien.no_rkm_medis=?",TCari.getText());
        
        // Memecah string berdasarkan tanda "-"
        String[] parts = tanggal_lahir.split("-");
        ...
        // Menampilkan umur
        String umur_pasien_saat_ini = "Umur pasien: " + age.getYears() + " tahun, " +
                           age.getMonths() + " bulan, dan " + age.getDays() + " hari.";
        System.out.println(umur_pasien_saat_ini);
        
        if(age.getYears() <= 17){
            if (JOptionPane.showConfirmDialog(null, "<html><div align='center'><font size='5' face='Tahoma' color='#825082'>Apakah umur anda dibawah 17 tahun?</font></div></html>\n"
                    + "<html><div align='center'><font size='15' face='Tahoma' color='#825082'>"+umur_pasien_saat_ini+"</font></div></html>\n"
                    + "<html><div align='center'><font size='15' face='Tahoma' color='#825082'>No HP: "+noHP+"</font></div></html>\n"
                    + "", "KONFIRMASI",
                    JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                try {
                    cek_pendaftaran_bpjs(TCari.getText());
                } catch (SQLException ex) {
                    Logger.getLogger(FormBPJS.class.getName()).log(Level.SEVERE, null, ex);
                }
            }else{
                // <html><h1>Silahkan ke petugas pendaftaran untuk memperbarui data diri Anda.</h1></html>
                JOptionPane.showMessageDialog(null, "<html><div align='center'><font size='15' face='Tahoma' color='#825082'>Silahkan ke petugas pendaftaran untuk memperbarui data diri Anda.</font></div></html>\n");
                dispose();
            }
        }else{
            if (JOptionPane.showConfirmDialog(null, "<html><div align='center'><font size='5' face='Tahoma' color='#825082'>Apakah nomor KTP dan Telp Anda sudah sesuai?</font></div></html>\n"
                    + "<html><div align='center'><font size='15' face='Tahoma' color='#825082'>KTP: "+noKTP+"</font></div></html>\n"
                    + "<html><div align='center'><font size='15' face='Tahoma' color='#825082'>No HP: "+noHP+"</font></div></html>\n"
                    + "", "KONFIRMASI",
                    JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                try {
                    form = new sikulixScreen(new javax.swing.JFrame(), true);
                    form.setSize(600, anjunganHeight);
                    form.setLocationRelativeTo(jPanel4);
                    form.setAlwaysOnTop(false);
                    form.setNoRm(TCari.getText());
