...
public final class DlgKamar extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        panelGlass4.add(jLabel1);
        jLabel1.setBounds(640, 12, 80, 23);

        kategori_tarif_ina.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Kamar / Akomodasi", "Rawat Intensif" }));
        kategori_tarif_ina.setName("kategori_tarif_ina"); // NOI18N
        panelGlass4.add(kategori_tarif_ina);
        kategori_tarif_ina.setBounds(725, 12, 160, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            String query = "INSERT INTO mapping_tarif_ina_kamar (kd_kamar, kategori_tarif)\n" +
        ...
        if(TKd.getText().trim().equals("")){
            ...
            if(tbKamar.getSelectedRow()>-1){
                ...
                try {
                    ...
                    rs = ps.executeQuery();
                    if(rs.next()){
                        String id_mapping = rs.getString("id_mapping_tarif_ina_kamar");
                        String query_update = "UPDATE mapping_tarif_ina_kamar SET kategori_tarif = '"+kategori_tarif_ina.getSelectedItem().toString()+"' WHERE id_mapping_tarif_ina_kamar = '"+id_mapping+"'";
                        ...
                        String query_mapping = "INSERT INTO mapping_tarif_ina_kamar (kd_kamar, kategori_tarif) VALUES ('"+TKd.getText()+"', '"+kategori_tarif_ina.getSelectedItem().toString()+"')";
    ...
    */
    ...
    public void tampil() {
        ...
        try{     
            ps=koneksi.prepareStatement("select \n" +
                                        ...
                                        "  kamar.kd_bangsal, \n" +
                                        "  bangsal.nm_bangsal, \n" +
                                        "  mapping_tarif_ina_kamar.kategori_tarif, \n" +
                                        "  kamar.kelas, \n" +
                                        "  kamar.trf_kamar, \n" +
                                        "  kamar.status \n" +
                                        "from \n" +
                                        ...
                                        "  left join mapping_tarif_ina_kamar on kamar.kd_kamar = mapping_tarif_ina_kamar.kd_kamar \n" +
                                        "where \n" +
                                        "  kamar.statusdata = '1' \n" +
                                        "  and bangsal.nm_bangsal like ? \n" +
                                        "  and kamar.status like ?"+
            ...
            try {                
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("kelas"),
                        rs.getDouble("trf_kamar"),
                        rs.getString("status")
                    });
                }
                // table autofit
                for (int col = 0; col < tbKamar.getColumnCount(); col++) {
                    TableColumn column = tbKamar.getColumnModel().getColumn(col);
                    int maxWidth = 50; // minimal lebar kolom

                    for (int row = 0; row < tbKamar.getRowCount(); row++) {
                        TableCellRenderer renderer = tbKamar.getCellRenderer(row, col);
                        Component comp = tbKamar.prepareRenderer(renderer, row, col);
    ...
    private void getData() {
        ...
        if(row!= -1){
            ...
            if(!tbKamar.getValueAt(tbKamar.getSelectedRow(),4).toString().equals("-")){
                kategori_tarif_ina.setSelectedItem(tbKamar.getValueAt(tbKamar.getSelectedRow(),4).toString());
            }else{
                kategori_tarif_ina.setSelectedItem("Kamar / Akomodasi");
            }
            Kelas.setSelectedItem(tbKamar.getValueAt(row,5).toString());
            TTarif.setText(Valid.SetAngka2(Double.parseDouble(tbKamar.getValueAt(row,6).toString())));
            CmbStatus.setSelectedItem(tbKamar.getValueAt(row,7).toString());
        }
