...
public final class DlgJnsPerawatanOperasi extends javax.swing.JDialog {
    ...
    private PreparedStatement pstampil, ps;
    ...
    private void initComponents() {
        ...
        kategori_tarif_ina = new javax.swing.JComboBox<>();
        ...
        jLabel1.setText("Mapping INA:");
        jLabel1.setName("jLabel1"); // NOI18N
        FormInput.add(jLabel1);
        jLabel1.setBounds(740, 12, 80, 23);

        kategori_tarif_ina.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Prosedur Bedah", "Keperawatan" }));
        kategori_tarif_ina.setName("kategori_tarif_ina"); // NOI18N
        FormInput.add(kategori_tarif_ina);
        kategori_tarif_ina.setBounds(825, 12, 160, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            String query = "INSERT INTO mapping_tarif_ina_paket_operasi (kode_paket, kategori_tarif)\n" +
                                "VALUES ('"+TKd.getText()+"', '"+kategori_tarif_ina.getSelectedItem().toString()+"');";
            try {
                ...
                if(affected_row > 0){
                    BtnCariActionPerformed(evt);
                    emptTeks();
                    notif_auto_close("Mapping tarif INA berhasil. ");
                }else{
                    notif_auto_close("Mapping tarif INA gagal. Silahkan hubungi IT.");
                }
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            if(tbJnsPerawatan.getSelectedRow()!= -1){
                ...
                String query_cek = "SELECT id_mapping_tarif_ina_paket_operasi FROM mapping_tarif_ina_paket_operasi WHERE kode_paket = '"+TKd.getText()+"'";
                try {
                    ps = koneksi.prepareStatement(query_cek);
                    rs = ps.executeQuery();
                    if(rs.next()){
                        String id_mapping = rs.getString("id_mapping_tarif_ina_paket_operasi");
                        String query_update = "UPDATE mapping_tarif_ina_paket_operasi SET kategori_tarif = '"+kategori_tarif_ina.getSelectedItem().toString()+"' WHERE id_mapping_tarif_ina_paket_operasi = '"+id_mapping+"'";
                        ps = koneksi.prepareStatement(query_update);
                        int updated_row = ps.executeUpdate();
                        if(updated_row > 0){
                            notif_auto_close("Update mapping tarif berhasil");
                        }else{
                            notif_auto_close("Update mapping tarif tidak berhasil!");
                        }
                    }else{
                        String query_mapping = "INSERT INTO mapping_tarif_ina_paket_operasi (kode_paket, kategori_tarif) VALUES ('"+TKd.getText()+"', '"+kategori_tarif_ina.getSelectedItem().toString()+"')";
                        ps = koneksi.prepareStatement(query_mapping);
                        int added_row = ps.executeUpdate();
                        if(added_row > 0){
                            notif_auto_close("Mapping tarif berhasil ditambahkan.");
                        }else{
                            notif_auto_close("Mapping tarif tidak berhasil ditambahkan. Silahkan cek kembali/hubungi IT.");
                        }
                    }
                } catch (SQLException ex) {
                    Logger.getLogger(DlgJnsPerawatanRalan.class.getName()).log(Level.SEVERE, null, ex);
...
private void TOmloop1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TOmloop1KeyPressed
    ...
    */
    ...
    private javax.swing.JComboBox<String> kategori_tarif_ina;
        ...
        try{    
            pstampil=koneksi.prepareStatement(
                       "select \n" +
                        ...
                        "from \n" +
                        "  paket_operasi \n" +
                        "  inner join penjab on penjab.kd_pj = paket_operasi.kd_pj \n" +
                        "  left join mapping_tarif_ina_paket_operasi on paket_operasi.kode_paket = mapping_tarif_ina_paket_operasi.kode_paket \n" +
                        "where \n" +
                        "  paket_operasi.status = '1' \n" +
                        "  and paket_operasi.kode_paket like ? \n" +
                        "  or paket_operasi.status = '1' \n" +
                        "  and paket_operasi.nm_perawatan like ? \n" +
                        "  or paket_operasi.status = '1' \n" +
                        "  and paket_operasi.kelas like ? \n" +
                        "  or paket_operasi.status = '1' \n" +
            ...
            try{
                ...
                while(rs.next()){                    
                    tabMode.addRow(new Object[]{
                                   ...
                                   rs.getString("kategori_tarif") == null ? "-" : rs.getString("kategori_tarif"),
                ...
                // table autofit
                for (int col = 0; col < tbJnsPerawatan.getColumnCount(); col++) {
                    TableColumn column = tbJnsPerawatan.getColumnModel().getColumn(col);
                    int maxWidth = 50; // minimal lebar kolom

                    for (int row = 0; row < tbJnsPerawatan.getRowCount(); row++) {
                        TableCellRenderer renderer = tbJnsPerawatan.getCellRenderer(row, col);
                        Component comp = tbJnsPerawatan.prepareRenderer(renderer, row, col);
    ...
    private void getData() {
        if(tbJnsPerawatan.getSelectedRow()!= -1){
            ...
            if(!tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),3).toString().equals("-")){
                kategori_tarif_ina.setSelectedItem(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),3).toString());
            }else{
                kategori_tarif_ina.setSelectedItem("Prosedur Bedah");
            }
            Kategori.setSelectedItem(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),4).toString());
            TOperator1.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),5).toString())));
            TOperator2.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),6).toString())));
            TOperator3.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),7).toString())));
            TAsisOperator1.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),8).toString())));
            TAsisOperator2.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),9).toString())));
            TAsisOperator3.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),10).toString())));
            TInstrumen.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),11).toString())));
            TAnastesi.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),12).toString())));
            TAsisAnastesi.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),13).toString())));
            TAsisAnastesi1.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),14).toString())));
            TAnak.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),15).toString())));
            TResusitas.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),16).toString())));
            TBidan1.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),17).toString())));
            TBidan2.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),18).toString())));
            TBidan3.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),19).toString())));
            TPerawatLuar.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),20).toString())));
            TAlat.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),21).toString())));
            TSewaOK.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),22).toString())));
            TAkomodasi.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),23).toString())));
            TBagianRS.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),24).toString())));
            TOmloop1.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),25).toString())));
            TOmloop2.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),26).toString())));
            TOmloop3.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),27).toString())));
            TOmloop4.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),28).toString())));
            TOmloop5.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),29).toString())));
            TSarpras.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),30).toString())));
            TdrPJAnak.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),31).toString())));
            TdrUmum.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),32).toString())));
            TTotal.setText(Valid.SetAngka2(Double.parseDouble(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),33).toString())));
            ...
            nmpnj.setText(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),34).toString());
            if(!tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),35).toString().equals("-")){
                Kelas.setSelectedItem(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),35).toString());
            }else{
