    ...
    public void tampil(String order) {         
        try{
            pspemeriksaan=koneksi.prepareStatement(
                    ...
                    "  left join hasil_pacs_radiologi on permintaan_pemeriksaan_radiologi.noorder=hasil_pacs_radiologi.no_order and jns_perawatan_radiologi.kd_jenis_prw=hasil_pacs_radiologi.kd_jenis_prw \n"+
                    "where \n" +
            ...
            try {
                ...
                rs=pspemeriksaan.executeQuery();          
//                pspemeriksaan2=koneksi.prepareStatement(
//                            "select hasil_pacs_radiologi.* from hasil_pacs_radiologi where kd_jenis_prw=? and no_order=?");
                
                // ambil hasil pemeriksaan
                PreparedStatement ps_detail_radiologi;
                String no_rawat_pacs;
                String no_order_pacs;
                String kd_jenis_prw_pacs;
                String hasil = "";
                ResultSet rs_detail_radiologi;
                String query_detail_radiologi;
                
                while(rs.next()){                
                    no_rawat_pacs = rs.getString("no_rawat");
                    no_order_pacs = rs.getString("no_order");
                    kd_jenis_prw_pacs = rs.getString("kd_jenis_prw");
                    query_detail_radiologi = ""
                            + "SELECT "
                            + "* "
                            + "FROM "
                            + "hasil_pacs_radiologi "
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getDouble(9),
                        rs.getDouble(10),
                        rs.getDouble("proyeksi"),
                        rs.getDouble("kV"),
                        rs.getDouble("mAS"),
                        rs.getDouble("FFD"),
                        rs.getDouble("BSF"),
                        rs.getDouble("inak"),
                        rs.getDouble("jml_penyinaran"),
                        rs.getDouble("dosis")
                    });
                }
                
                HasilPeriksa.setText(hasil);
            } catch (Exception e) {
                System.out.println("Notifikasi 1 : "+e);
            } finally{
                if(rs!=null){
