...
public final class DlgPeriksaLaboratorium extends javax.swing.JDialog {
    ...
    private PreparedStatement pstindakan,pstampil,pstampil2,pstampil3,pstampil4,pslica,
            psset_tarif,pssetpj,psrekening, ps;
    private ResultSet rstindakan,rstampil,rsset_tarif,rssetpj,rsrekening,rslica, rs;
                    ...
                    if((!tbPemeriksaan.getValueAt(i,6).toString().equals(""))&&tbPemeriksaan.getValueAt(i,0).toString().equals("true")){                                
                        try {
                            if(Sequel.menyimpantf2("detail_periksa_lab","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","ID Template",16,new String[]{
                                    ...
                                    tbPemeriksaan.getValueAt(i,15).toString(), // kd_jenis_prw
                                    Valid.SetTgl(Tanggal.getSelectedItem()+""), // tgl_periksa 
                                    CmbJam.getSelectedItem()+":"+CmbMenit.getSelectedItem()+":"+CmbDetik.getSelectedItem(), // jam 
                                    tbPemeriksaan.getValueAt(i,6).toString(), // id_template 
                                    tbPemeriksaan.getValueAt(i,2).toString(), // nilai
                                    tbPemeriksaan.getValueAt(i,4).toString(), // nilai_rujukan 
                                    tbPemeriksaan.getValueAt(i,5).toString(), // keterangan 
                                    tbPemeriksaan.getValueAt(i,8).toString(), // bagian_rs 
                                    tbPemeriksaan.getValueAt(i,9).toString(), // bhp 
                                    tbPemeriksaan.getValueAt(i,10).toString(), // bagian_perujuk 
                                    tbPemeriksaan.getValueAt(i,11).toString(), // bagian_dokter 
                                    tbPemeriksaan.getValueAt(i,12).toString(), // bagian_laborat 
                                    tbPemeriksaan.getValueAt(i,13).toString(), // kso 
                                    tbPemeriksaan.getValueAt(i,14).toString(), // menejemen 
                                    tbPemeriksaan.getValueAt(i,7).toString() // biaya_item 
                                })==true){
                                // kirim ke view hasil lab
                                // noorder;
                                try {
                                    ps=koneksi.prepareStatement("insert into view_hasil_lab "
                                            + "(no_rawat, noorder, tgl_permintaan, jam_hasil, kd_jenis_prw, id_template, nilai_rujukan, keterangan, dokter_perujuk, status) "
                                            + "values "
                                            + "("
                                                + "'"+TNoRw.getText()+"', " // no_rawat
                                                + "'"+noorder+"', " // noorder
                                                + "'"+Valid.SetTgl(Tanggal.getSelectedItem()+"")+"', " // tgl_permintaan
                                                + "'"+CmbJam.getSelectedItem()+":"+CmbMenit.getSelectedItem()+":"+CmbDetik.getSelectedItem()+"', " // jam_hasil
                                                + "'"+tbPemeriksaan.getValueAt(i,15).toString()+"', " // kd_jenis_prw
                                                + "'"+tbPemeriksaan.getValueAt(i,6).toString()+"', " // id_template
                                                + "'"+tbPemeriksaan.getValueAt(i,4).toString()+"', " // nilai_rujukan
                                                + "'"+tbPemeriksaan.getValueAt(i,5).toString()+"', " // keterangan
                                                + "'"+KodePerujuk.getText()+"', " // dokter_perujuk
                                                + "'"+Sequel.cariIsi("SELECT status FROM `permintaan_lab` WHERE noorder = '"+noorder+"' AND no_rawat = '"+TNoRw.getText()+"' ORDER BY `tgl_permintaan` DESC")+"' " // status
                                            + ") "
                                            + "");
                                    try{          
                                        ps.executeUpdate();
                                    }catch(Exception e){
                                        System.out.println("Notifikasi error log kirim ke view hasil lab: "+e);            
                                    }finally{
                                        if(ps != null){
                                            ps.close();
                                        }                
