        ...
        try {
            ...
            if(NoRawat.getText().equals("")&&kdmem.getText().equals("")&&kdptg.getText().equals("")&&TCari.getText().equals("")){
                ps=koneksi.prepareStatement(
                    ...
                    "  left join view_anjungan on periksa_lab.no_rawat = view_anjungan.param_2\n" +
                    "where \n" +
                    "  periksa_lab.kategori = 'PK' \n" +
                    "  and periksa_lab.tgl_periksa between ? \n" +
                    "  and ? \n" +
                    "group by \n" +
                    "  concat(\n" +
                    "    periksa_lab.no_rawat, periksa_lab.tgl_periksa, \n" +
                    "    periksa_lab.jam\n" +
                    "  ) \n" +
                    "order by \n" +
            ...
            try {
                ...
                rs=ps.executeQuery();
                System.out.println("query ps: "+ps);
                ttl=0;
                while(rs.next()){
                    kamar=Sequel.cariIsi("select ifnull(kamar_inap.kd_kamar,'') from kamar_inap where kamar_inap.no_rawat='"+rs.getString("no_rawat")+"' order by kamar_inap.tgl_masuk desc limit 1");
                    if(!kamar.equals("")){
                    ...
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("no_rkm_medis")+" "+rs.getString("nm_pasien")+" ("+kamar+" : "+namakamar+")",
                        rs.getString("nama"),
                        rs.getString("tgl_periksa"),
                        rs.getString("jam"),
