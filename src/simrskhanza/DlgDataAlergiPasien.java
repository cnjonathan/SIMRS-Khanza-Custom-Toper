...
public class DlgDataAlergiPasien extends javax.swing.JDialog {
    ...
    public DlgDataAlergiPasien(java.awt.Frame parent, boolean modal) {
        ...
        tbDaftarPasien.setModel(tabMode);

        //tbObat.setDefaultRenderer(Object.class, new WarnaTable(panelJudul.getBackground(),tbObat.getBackground()));
        tbDaftarPasien.setPreferredScrollableViewportSize(new Dimension(500, 500));
        tbDaftarPasien.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);

        for (i = 0; i < 16; i++) {
            TableColumn column = tbDaftarPasien.getColumnModel().getColumn(i);
            if (i == 0) {
        ...
        tbDaftarPasien.setDefaultRenderer(Object.class, new WarnaTable());
    ...
    private void initComponents() {
        ...
        TNIKpegawai = new javax.swing.JTextField();
        internalFrame1 = new widget.InternalFrame();
        Scroll = new widget.ScrollPane();
        tbDaftarPasien = new widget.Table();
        ...
        TNIKpegawai.setName("TNIKpegawai"); // NOI18N
        ...
        tbDaftarPasien.setToolTipText("Silahkan klik untuk memilih data yang mau diedit ataupun dihapus");
        tbDaftarPasien.setName("tbDaftarPasien"); // NOI18N
        tbDaftarPasien.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tbDaftarPasienMouseClicked(evt);
            }
        });
        tbDaftarPasien.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                tbDaftarPasienKeyPressed(evt);
            }
        });
        Scroll.setViewportView(tbDaftarPasien);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoRw.getText().trim().equals("") || NoRM.getText().trim().equals("") || NmPasien.getText().trim().equals("")) {
            ...
            if (Sequel.menyimpantf("alergi_pasien", "?,?,?,?,?,?,?,?", "No.Rawat", 8, new String[]{
                NoRw.getText(), Sequel.cariIsi("select CURRENT_TIMESTAMP()"), AlergiCode.getText(), cmbKategory.getSelectedItem().toString(), TNIKpegawai.getText(), TKeterangan.getText(), ReaksiCode.getText(), cmbSeverity.getSelectedItem().toString()
            }) == true) {
                tampil();
                emptTeks();
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (tbDaftarPasien.getSelectedRow() > -1) {
            if (akses.getkode().equals("Admin Utama")) {
                if (Sequel.queryu2tf("delete from alergi_pasien where no_rawat=?", 1, new String[]{
                    tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 0).toString()
                }) == true) {
                    tampil();
                    emptTeks();
                } else {
                ...
                if (akses.getkode().equals(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 14).toString())) {
                    if (Sequel.queryu2tf("delete from alergi_pasien where no_rawat=?", 1, new String[]{
                        tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 0).toString()
                    }) == true) {
                        tampil();
    ...
    private void tbDaftarPasienMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDaftarPasienMouseClicked
...
}//GEN-LAST:event_tbDaftarPasienMouseClicked

    private void tbDaftarPasienKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDaftarPasienKeyPressed
        if (tabMode.getRowCount() != 0) {
...
}//GEN-LAST:event_tbDaftarPasienKeyPressed
    ...
    private void btnVaksinCode1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnVaksinCode1ActionPerformed
        mode = "reaksialergi";
        allergycode.isCek(mode);
        allergycode.setSize(internalFrame1.getWidth() - 20, internalFrame1.getHeight() - 20);
        allergycode.setLocationRelativeTo(internalFrame1);
        allergycode.setVisible(true);
        allergycode.tampilReaksi();
    ...
    private javax.swing.JTextField TNIKpegawai;
    ...
    private widget.Table tbDaftarPasien;
    ...
    public void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    "FROM "+
                    "  alergi_pasien "+
                    "  LEFT JOIN reg_periksa ON alergi_pasien.no_rawat = reg_periksa.no_rawat "+
                    "  LEFT JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "+
                    "  LEFT JOIN penjab ON reg_periksa.kd_pj = penjab.kd_pj "+
                    "  LEFT JOIN pegawai ON alergi_pasien.nippetugas = pegawai.nik "+
                    "  LEFT JOIN satu_sehat_ref_allergy ON alergi_pasien.allergy_code = satu_sehat_ref_allergy.kode "+
                    "  LEFT JOIN satu_sehat_ref_allergy_reaction ON alergi_pasien.reactioncode = satu_sehat_ref_allergy_reaction.kode "+
                    "where "+
                    "  alergi_pasien.tgl_perawatan between ? "+
                    "  and ?"+
                    ""
            ...
            try {
                ...
                System.out.println("Query Data Alergi Pasien: "+ps.toString());
                rs = ps.executeQuery();
                while (rs.next()) {
                    //"No.Rawat", "No.RM", "Nama Pasien", "J.K.", "Umur", "No.Telp", "Cara Bayar", "TANGGAL", "KODE", "ALERGI", "KATEGORI", "REAKSI KODE", "SEVERITY", "NOTE", "NIP", "Nama Petugas"
    ...
    private void getData() {
        // "No.Rawat", "No.RM", "Nama Pasien", "J.K.", "Umur", "No.Telp", "Cara Bayar", "TANGGAL", "KODE", "ALERGI", "KATEGORI", "REAKSI KODE", "SEVERITY", "NOTE", "NIP", "Nama Petugas"
        if (tbDaftarPasien.getSelectedRow() != -1) {
            NoRw.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 0).toString());
            NoRM.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 1).toString());
            NmPasien.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 2).toString());
            AlergiCode.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 8).toString());
            AlergySystem.setText(Sequel.cariIsi("select satu_sehat_ref_allergy.system from satu_sehat_ref_allergy where satu_sehat_ref_allergy.kode='" + AlergiCode.getText() + "'"));
            AlergyDisplay.setText(Sequel.cariIsi("select satu_sehat_ref_allergy.display from satu_sehat_ref_allergy where satu_sehat_ref_allergy.kode='" + AlergiCode.getText() + "'"));
            ReaksiCode.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 11).toString());
            ReaksiSystem.setText(Sequel.cariIsi("select satu_sehat_ref_allergy_reaction.system from satu_sehat_ref_allergy_reaction where satu_sehat_ref_allergy_reaction.kode='" + ReaksiCode.getText() + "'"));
            ReaksiDisplay.setText(Sequel.cariIsi("select satu_sehat_ref_allergy_reaction.display from satu_sehat_ref_allergy_reaction where satu_sehat_ref_allergy_reaction.kode='" + ReaksiCode.getText() + "'"));
            TKeterangan.setText(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 13).toString());
            cmbKategory.setSelectedItem(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 10).toString());
            cmbSeverity.setSelectedItem(tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 12).toString());
        }
    }

    public void setNoRm(String norwt, String norm, String nama, String nikpegawai) {
        NoRw.setText(norwt);
        NoRM.setText(norm);
        NmPasien.setText(nama);
        TCari.setText(norwt);
        TNIKpegawai.setText(nikpegawai);
        aktif = false;
    }
    ...
    private void ganti() {
        ...
        if (Sequel.mengedittf("alergi_pasien", "no_rawat=?", "no_rawat=?,tgl_perawatan=?,allergy_code=?,category=?,nippetugas=?,note=?,reactioncode=?,severity=?", 9, new String[]{
            NoRw.getText(), Sequel.cariIsi("select CURRENT_TIMESTAMP()"), AlergiCode.getText(), cmbKategory.getSelectedItem().toString(), TNIKpegawai.getText(), TKeterangan.getText(), ReaksiCode.getText(), cmbSeverity.getSelectedItem().toString(), tbDaftarPasien.getValueAt(tbDaftarPasien.getSelectedRow(), 0).toString()
        }) == true) {
            tampil();
            emptTeks();
        }
    ...
    public JTable getTable() {
        return tbDaftarPasien;
    }
    
    public String getReaksiDisplay(){
        return "Kategori: "+cmbKategory.getSelectedItem().toString()+"; Display: "+ReaksiDisplay.getText()+"; Reaksi: "+ReaksiDisplay.getText()+"; Keterangan: "+TKeterangan.getText()+"; ";
    }
