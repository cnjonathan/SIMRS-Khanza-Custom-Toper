...
public final class DlgCariPoli extends javax.swing.JDialog {
    ...
    public DlgCariPoli(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Kode Unit","Location ID","Nama Unit","Registrasi Baru","Registrasi Lama"};
    ...
    */
    ...
    private void tampil() {
        ...
        try {
            ...
            iyem="";
            ps=koneksi.prepareStatement("select poliklinik.*, satu_sehat_mapping_lokasi_ralan.id_organisasi_satusehat, satu_sehat_mapping_lokasi_ralan.id_lokasi_satusehat from poliklinik left join satu_sehat_mapping_lokasi_ralan ON poliklinik.kd_poli = satu_sehat_mapping_lokasi_ralan.kd_poli where poliklinik.status='1'");
            try{           
                rs=ps.executeQuery();
                while(rs.next()){
                    ...
                    iyem=iyem+"{\"KodeUnit\":\""+rs.getString(1)
                                +"\",\"Location ID\":\""+rs.getString(7)
                                +"\",\"NamaUnit\":\""+rs.getString(2)
                                +"\",\"RegistrasiBaru\":\""+rs.getString(3)
                                +"\",\"RegistrasiLama\":\""+rs.getString(4)
                            +"\"},";
                }
            }catch(Exception e){
            ...
            if(response.isArray()){
                if(TCari.getText().trim().equals("")){
                    for(JsonNode list:response){
                        tabMode.addRow(new Object[]{
                            list.path("KodeUnit").asText(),
                            list.path("Location ID").asText(),
                            list.path("NamaUnit").asText(),
                            list.path("RegistrasiBaru").asText(),
                            list.path("RegistrasiLama").asText()
                        });
                    }
                }else{
                    for(JsonNode list:response){
