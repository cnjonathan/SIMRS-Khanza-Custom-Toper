    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(TNoRw.getText().equals("")||TNoRM.getText().equals("")||TPasien.getText().equals("")){
            ...
            if((jmlparsial>0)&&status.equals("Ralan")){   
                System.out.println("if((jmlparsial>0)&&status.equals(RALAN))");
                simpan();
            }else{
                if(Sequel.cariRegistrasi(TNoRw.getText())>0){
                    ...
                    System.out.println("else (jmlparsial>0)&&status.equals(RALAN)");
                    simpan();
                }
            }                    
        }
...
private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
    ...
    */
    ...
    private void tampiltarif() {     
        System.out.println("tampiltarif");
        try{
            jml=0;
            for(i=0;i<tbTarif.getRowCount();i++){
                if(tbTarif.getValueAt(i,0).toString().equals("true")){
    ...
    private void tampiltarif(String order) {     
        System.out.println("tampiltarif: 1850");
        try{
            Valid.tabelKosong(tabMode2);
            pstindakan=koneksi.prepareStatement(
                "select jns_perawatan_lab.kd_jenis_prw,jns_perawatan_lab.nm_perawatan,jns_perawatan_lab.total_byr,"+
    ...
    private void simpanlab() {
        ...
        try {                    
            ...
            for(i=0;i<tbTarif.getRowCount();i++){ 
                if(tbTarif.getValueAt(i,0).toString().equals("true")){
                    System.out.println("tbTarif.getValueAt(i,0).toString().equals(true)");
                    if(Sequel.menyimpantf2("periksa_lab","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Kode Pemeriksaan",17,new String[]{
                            TNoRw.getText(),//1
                            KdPtg.getText(),//2
                            tbTarif.getValueAt(i,1).toString(),//3
                            Valid.SetTgl(Tanggal.getSelectedItem()+""),//4
                            CmbJam.getSelectedItem()+":"+CmbMenit.getSelectedItem()+":"+CmbDetik.getSelectedItem(),//5
                            KodePerujuk.getText(),//6
                            tbTarif.getValueAt(i,4).toString(),//7
                            tbTarif.getValueAt(i,5).toString(),//8
                            tbTarif.getValueAt(i,6).toString(),//9
                            tbTarif.getValueAt(i,7).toString(),//10
                            tbTarif.getValueAt(i,8).toString(),//11
                            tbTarif.getValueAt(i,9).toString(),//12
                            tbTarif.getValueAt(i,10).toString(),//13
                            tbTarif.getValueAt(i,3).toString(),//14
                            KodePj.getText(),//15
                            status,//16  
                            "PA"//17
                        })==true){
                            System.out.println("Sequel.menyimpantf2 periksa_lab");
                            if(!noorder.equals("")){
            ...
            if(sukses==true){
                System.out.println("sukses==true");
                for(i=0;i<tbPemeriksaan.getRowCount();i++){ 
                    if(!tbPemeriksaan.getValueAt(i,2).toString().equals("")){  
                        System.out.println("!tbPemeriksaan.getValueAt(i,2).toString() equal null");
                        if(Sequel.menyimpantf2("detail_periksa_labpa","?,?,?,?,?,?,?,?,?,?","Pemeriksaan Lab",10,new String[]{
                                TNoRw.getText(), //no_rawat 
                                tbPemeriksaan.getValueAt(i,6).toString(), //kd_jenis_prw 
                                ...
                                tbPemeriksaan.getValueAt(i,1).toString(), //Diagnosa Klinis
                                tbPemeriksaan.getValueAt(i,2).toString(), //Makroskopik
                                tbPemeriksaan.getValueAt(i,3).toString(), //Mikroskopik
                                tbPemeriksaan.getValueAt(i,4).toString(), //Kesimpulan
                                tbPemeriksaan.getValueAt(i,5).toString(), // Kesan
                                ""
                            })==false){
                            System.out.println("Sequel.menyimpantf2 detail_periksa_labpa");
                            sukses=false;
                        }
                    }                        
                }
