            ...
            if(tbDokter.getSelectedRow()>-1){
                ...
                if(Sequel.menyimpantf2("pemeriksaan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Data",21,new String[]{
                    ...
                    if((tabModeObatUmum.getRowCount()>0)||(tabModeObatRacikan.getRowCount()>0)){
                        nomor=Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(resep_obat.no_resep,4),signed)),0) from resep_obat where LEFT(resep_obat.no_resep,8)='"+tanggaldilakukan.replaceAll("-","")+"' ",tanggaldilakukan.replaceAll("-",""),4);    
