...
public class MasterTriasePemeriksaan extends javax.swing.JDialog {
    ...
    public MasterTriasePemeriksaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Kode","Pemeriksaan","Status Aktif"};
    ...
    private void initComponents() {
        ...
        cmb_status.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Non Aktif", "Aktif" }));
        cmb_status.setSelectedIndex(1);
        cmb_status.setName("cmb_status"); // NOI18N
        panelGlass7.add(cmb_status);
        cmb_status.setBounds(530, 10, 120, 22);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            String query = "INSERT INTO master_triase_pemeriksaan ("
                    + "kode_pemeriksaan, "
                    + "nama_pemeriksaan, "
                    + "status "
                    + ")\n"
                    ...
                    + "'"+TKd.getText()+"', "
                    + "'"+TNm.getText()+"', "
                    + "'"+cmb_status.getSelectedIndex()+"' "
                    + ");";
            try {
                ps = koneksi.prepareStatement(query);
                int affected_row = ps.executeUpdate();
                if(affected_row > 0){
                    notif_auto_close("Simpan Resume Berhasil. ");
                    tampil();
                    emptTeks();
                }else{
                ...
                Logger.getLogger(MasterTriasePemeriksaan.class.getName()).log(Level.SEVERE, null, ex);
            }finally{
                if(ps!=null){
                    try {
                        ps.close();
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TKd.getText().trim().equals("")){
            ...
            if(tbSpesialis.getSelectedRow()> -1){
//                Sequel.mengedit("master_triase_pemeriksaan","kode_pemeriksaan=?","kode_pemeriksaan=?,nama_pemeriksaan=?,status=?",4,new String[]{
//                    TKd.getText(),TNm.getText(),tbSpesialis.getValueAt(tbSpesialis.getSelectedRow(),0).toString(),cmb_status.getSelectedIndex()+""
//                });
//                if(tabMode.getRowCount()!=0){tampil();}
                try {
                    String query_update = "UPDATE master_triase_pemeriksaan SET  \n"
                                            + "nama_pemeriksaan = '"+TNm.getText()+"', \n" 
                                            + "status = '"+cmb_status.getSelectedIndex()+"' \n" 
                                            + "WHERE kode_pemeriksaan = '"+TKd.getText()+"';";
                    ps=koneksi.prepareStatement(query_update);
                    int affected_row = ps.executeUpdate();
                    if(affected_row > 0){
                        notif_auto_close("Update data triase berhasil. ");
                        emptTeks();
                        tampil();
                    }else{
                        notif_auto_close("Update data triase gagal. Silahkan hubungi IT.");
                    }
                } catch (SQLException ex) {
                    Logger.getLogger(MasterTriasePemeriksaan.class.getName()).log(Level.SEVERE, null, ex);
                }finally{
    ...
    */
    ...
    private void getData() {
        if(tbSpesialis.getSelectedRow()!= -1){
            ...
            int status = 0;
            if(tbSpesialis.getValueAt(tbSpesialis.getSelectedRow(),2).toString().equals("Non Aktif")){
                status = 0;
            }else{
                status = 1;
            }
