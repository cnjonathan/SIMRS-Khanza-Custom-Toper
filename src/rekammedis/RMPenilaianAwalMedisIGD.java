...
public final class RMPenilaianAwalMedisIGD extends javax.swing.JDialog {
    ...
    private PreparedStatement ps, ps_triase;
    private ResultSet rs, rs_triase;
    private int i=0;
    ...
    public void cek_ttv_triase() {
        ...
        try {
            ...
            if(rs_triase.next()){
                ...
                String keluhan_utama = Sequel.cariIsi("SELECT keluhan_utama FROM data_triase_igdprimer WHERE no_rawat = '"+TNoRw.getText()+"'");
                if(keluhan_utama == null || keluhan_utama.equals("") || keluhan_utama.isEmpty()){
                    keluhan_utama = Sequel.cariIsi("SELECT anamnesa_singkat FROM data_triase_igdsekunder WHERE no_rawat = '"+TNoRw.getText()+"'");
                }
                KeluhanUtama.setText(keluhan_utama);
                // atur kecepatan scroll
                scrollInput.getVerticalScrollBar().setUnitIncrement(20);
            }
        } catch (SQLException ex) {
            Logger.getLogger(RMPenilaianAwalMedisIGD.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
